...
public:
    PurePursuit() : Node("pure_pursuit_node")
    {
        ...
        //for pf: check if necessary 
        // odo_subscription = this->create_subscription<nav_msgs::msg::Odometry>(
        //     "/pf/viz/inferred_pose", 10, std::bind(&PurePursuit::odom_callback, this, std::placeholders::_1));
...
private:
    ...
    const double CAR_WHEELBASE = 0.3; //m
    ...
    int target_waypoint_sim(std::vector<waypoints> &global_waypoints, int prev_target_idx, int target_window) {
        ...
        //loop through all initially, make sure 
        ...
        //Inital loop, only done once. Used to find closest node regardless of start
        if (prev_target_idx == -1) {
            double closest_dist = 1e10;
            int closest_idx = 0;
            
            for (int i = 0; i < n_size; i++) {
                float dx = global_waypoints[i].x - x_car;
                float dy = global_waypoints[i].y - y_car;
            ...
            // Update prev_target_idx so the window search below can use it as a starting point
            prev_target_idx = closest_idx;
        }

        //main loop, used to find the best waypoint 
